# Escenario forense digital práctico
...
Pertenece al equipo forense digital y está asignado para investigar un incidente relacionado con un sistema de Windows utilizando un volcado de memoria, una imagen de disco completa y artefactos rápidos de triaje.

- **Ubicación del volcado de memoria**: `C:\Users\johndoe\Desktop\memdump\PhysicalMemory.raw`
- **Ubicaciones de artefactos de triaje rápido**:
    - `C:\Users\johndoe\Desktop\kapefiles`
    - `C:\Users\johndoe\Desktop\files`
- **Ubicación de la imagen de disco completo**: `C:\Users\johndoe\Desktop\fulldisk.raw.001`
- **Ubicación de datos de imagen de disco completo analizado**: `C:\Users\johndoe\Desktop\MalwareAttack`

**Notas**:

- Al analizar con`Autopsy`, sugerimos firmemente acceder al caso desde`C:\Users\johndoe\Desktop\MalwareAttack`.
- Durante una investigación, es imperativo examinar los artefactos o la evidencia en un sistema especializado adaptado para tareas forenses. Sin embargo, en aras de la conveniencia, el análisis se realiza dentro del sistema afectado mismo.

---

# **Análisis de memoria con volatilidad V3**

El volcado de memoria del sistema afectado reside en`C:\Users\johndoe\Desktop\memdump\PhysicalMemory.raw`.

### **Identificación del perfil del volcado de memoria**

Comencemos por obtener detalles del sistema operativo y el kernel de la muestra de memoria de Windows que se está analizando, aprovechando la volatilidad`windows.info`complemento.
...
Volatilidad`windows.malfind`El complemento se puede usar para enumerar rangos de memoria de proceso que potencialmente contienen código inyectado de la siguiente manera.
...
Cuando un proceso asigna una página de memoria con`PAGE_EXECUTE_READWRITE`Permisos, esencialmente solicita la capacidad de ejecutar y escribir en esa región de memoria. En términos de Layman, el proceso dice: "Quiero poder ejecutar código desde aquí, pero también quiero que la flexibilidad cambie lo que ese código está en la marcha".

Ahora, ¿por qué eso levanta las cejas? Bueno, las aplicaciones legítimas generalmente segregan las tareas de ejecución de código y redacción de datos. Tendrán regiones específicas de memoria para ejecutar código (ejecutable) y separar regiones donde los datos se escriben o modifican. Esta separación es un principio de seguridad fundamental, asegurando que los datos no se ejecuten inadvertidamente o que las regiones ejecutables no sean manipuladas inesperadamente.

Sin embargo, muchos tipos de malware, especialmente aquellos que emplean técnicas de inyección de código, requieren la capacidad de escribir su carga útil en la memoria y luego ejecutarla. Asignando memoria con`PAGE_EXECUTE_READWRITE`Permisos, pueden escribir y posteriormente ejecutar código malicioso dentro de la misma región de memoria, haciendo que sus actividades maliciosas sean más simplificadas y eficientes.

En esencia, mientras que no todas las instancias de`PAGE_EXECUTE_READWRITE`es malicioso, su presencia es un fuerte indicador de malversación potencial, y es algo que nosotros, como analistas de seguridad vigilantes, debemos analizar de cerca.

### **Identificación de procesos de ejecución**

Enumeremos ahora los procesos presentes en esta imagen de memoria de Windows en particular a través de la volatilidad`windows.pslist`complemento de la siguiente manera.
...
Si queremos enumerar los procesos en un árbol en función de la identificación de su proceso principal, podemos hacerlo a través de la volatilidad`windows.pstree`complemento de la siguiente manera.
...
### **Identificación de líneas de comando de proceso**

Volatilidad`windows.cmdline`El complemento puede proporcionarnos una lista de argumentos de línea de comandos de proceso de la siguiente manera.
...
### **Memoria del proceso de vertido y aprovechando a Yara**

Ya debería ser obvio que el proceso`3648`Parece sospechoso. Para extraer todas las páginas de residentes de memoria en un proceso en un archivo individual, podemos usar la volatilidad`windows.memmap`complemento de la siguiente manera.
...
`pid.3648.dmp`se puede encontrar dentro del`c:\Users\johndoe\Desktop`Directorio del objetivo de esta sección para su conveniencia.

Para obtener más detalles sobre el proceso con ID`3648`, podemos emplear a Yara. Al aprovechar un bucle de PowerShell, podemos escanear sistemáticamente el volcado de procesos utilizando todas las reglas disponibles de la[https://github.com/neo23x0/signature-base/tree/master](https://github.com/Neo23x0/signature-base/tree/master)Repositorio de reglas de Yara, que se puede encontrar dentro del`C:\Users\johndoe\Desktop\yara-4.3.2-2150-win64\rules`directorio del objetivo de esta sección.

Escenario forense digital práctico

```powershell
PS C:\Users\johndoe> $rules = Get-ChildItem C:\Users\johndoe\Desktop\yara-4.3.2-2150-win64\rules | Select-Object -Property NamePS C:\Users\johndoe> foreach ($rule in $rules) {C:\Users\johndoe\Desktop\yara-4.3.2-2150-win64\yara64.exe C:\Users\johndoe\Desktop\yara-4.3.2-2150-win64\rules\$($rule.Name) C:\Users\johndoe\Desktop\pid.3648.dmp}HKTL_CobaltStrike_Beacon_Strings C:\Users\johndoe\Desktop\pid.3648.dmp
...
Notamos algunos éxitos relacionados con el[Marco de ataque de cobalto](https://www.cobaltstrike.com/).

### **Identificar DLL cargadas**

Al examinar las líneas de comando, identificamos argumentos que apuntan a`payload.dll`para el proceso`3648`, con el`Start`función que sirve como un signo claro de`payload.dll`Secución. Para promover nuestra comprensión de las DLL asociadas, podemos emplear la volatilidad`windows.dlllist`complemento de la siguiente manera.
...
Notamos`E:\payload.dll`en la salida de la volatilidad. Según su ubicación, suponemos que podría originarse en un USB externo o tal vez un archivo ISO montado. Asignaremos esta DLL para un análisis más profundo más adelante.

### **Identificación de asas**

A continuación, identifiquemos los archivos y las entradas de registro accedidas por el proceso sospechoso utilizando la volatilidad`windows.handles`complemento.

Cuando un proceso necesita leer o escribir en un archivo, no interactúa directamente con los datos del archivo en el disco. En su lugar, el proceso solicita al sistema operativo que abra el archivo y, a cambio, el sistema operativo proporciona un identificador de archivo. Este mango es esencialmente un boleto que otorga el permiso del proceso para realizar operaciones en ese archivo. Cada operación posterior, el proceso realiza en ese archivo, ya sea lectura, escritura o cierre, se realiza a través de este mango.

Las manijas abiertas pueden ser una mina de oro para analistas forenses. Al examinar la lista de manijas abiertas, podemos determinar a qué procesos accedían a qué archivos o claves de registro en un momento particular. Esto puede proporcionar información sobre el comportamiento del software potencialmente malicioso o las acciones de un usuario.
...
Es evidente (basado en`\Device\HarddiskVolume3\Users\johndoe\Desktop`) que el proceso tiene interacciones con ciertos archivos ubicados en el`Desktop`, que garantiza una mirada más cercana en breve.

### **Identificación de artefactos de red**

Alejarse de los procesos, volatilidad`windows.netstat`El complemento puede recorrer estructuras de seguimiento de red para ayudarnos a analizar los detalles de la conexión dentro de una imagen de memoria.
...
Para un análisis de red más exhaustivo, también podemos emplear la volatilidad`windows.netscan`complemento de la siguiente manera:
...
El proceso sospechoso (pid`3648`) se ha estado comunicando con`44.214.212.249`sobre el puerto`80`.

---

# **Imagen de disco/análisis de datos de triaje rápido**

### **Buscando palabras clave con autopsia**

Primero abramos`Autopsy`y acceder al caso desde`C:\Users\johndoe\Desktop\MalwareAttack`.

Ahora, para rastrear`payload.dll`En el disco, navegaremos`Autopsy`e iniciar una búsqueda del`payload.dll`Palabra clave, priorizar los resultados por su tiempo de creación.

![](https://academy.hackthebox.com/storage/modules/237/image78.png)

Entre los 29 hallazgos, el`Finance08062023.iso`archivo en el`Downloads`El directorio debe despertar nuestro interés (recuerde el DLL en el`E`¿conducir?). Podemos extraer este archivo para un escrutinio posterior, haciendo clic derecho y seleccionando`Extract File(s)`.

![](https://academy.hackthebox.com/storage/modules/237/image32.png)

Dada la presencia del archivo en el`Downloads`carpeta y una correspondiente`Chrome cache file` (`f_000003`) apuntando a cadenas similares, es plausible que el`ISO`El archivo se obtuvo a través de un navegador.

![](https://academy.hackthebox.com/storage/modules/237/image57.png)

### **Identificar información de descarga web y extrayendo archivos con autopsia**

Para extraer detalles de descarga web, aprovecharemos las capacidades de`ADS`. Dentro`Autopsy`, podemos acceder al`Downloads`directorio para localizar nuestro archivo. Aquí, el`.Zone.Identifier`información, cortesía del`Alternate Data Stream (ADS)`Atributos del archivo, es invaluable.

![](https://academy.hackthebox.com/storage/modules/237/image87.png)

Este identificador revela el origen de Internet del archivo, y podemos identificar el`HostUrl`de la cual lo malicioso`ISO`fue de origen.

![](https://academy.hackthebox.com/storage/modules/237/image21.png)

Corroborando nuestros hallazgos,`Autopsy`'s`Web Downloads`Los artefactos confirman que`Finance08062023.iso`fue obtenido de`letsgohunt[.]site`.

![](https://academy.hackthebox.com/storage/modules/237/image9_.png)

Al montar el extraído`ISO`Archivo, notamos que alberga un`DLL`y un archivo de acceso directo, que aprovecha`rundll32.exe`para activar`payload.dll`.

![](https://academy.hackthebox.com/storage/modules/237/image91.png)

### **Extracción de la configuración de Beacon de Cobalt Strike**

Dado nuestro conocimiento del uso del atacante de`Cobalt Strike`, podemos intentar extraer el[faro](https://www.cobaltstrike.com/product/features/beacon)configuración a través del[Script CobaltStrikeParser](https://github.com/Sentinel-One/CobaltStrikeParser), que se puede encontrar dentro del`C:\Users\johndoe\Desktop\CobaltStrikeParser-master\CobaltStrikeParser-master`Directorio del objetivo de esta sección de la siguiente manera.
...
### **Identificar la persistencia con autores**

Para los mecanismos de persistencia, inspeccionemos el`C:\Users\johndoe\Desktop\files\johndoe_autoruns.arn`archivo usando el`Autoruns`herramienta.

Dentro del`Logon`Sección, notamos un`LocalSystem`Entrada con los siguientes detalles:

- **Ruta de registro**: `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run`
- **Ruta de imagen**: `C:\ProgramData\svchost.exe`
- **Marca de tiempo**: `Thu Aug 10 11:25:51 2023`(Esta es una marca de tiempo local, UTC: 09:25:51)

![](https://academy.hackthebox.com/storage/modules/237/image2__.png)

Además, un extraño`photo433.exe`Ejecutable ha sido marcado.

`photo433.exe`se ha extraído durante el rápido proceso de triaje y reside dentro del`C:\Users\johndoe\Desktop\kapefiles\auto\C%3A\Users\johndoe\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\`directorio del objetivo de esta sección.

Su hash SHA256 se puede identificar utilizando PowerShell de la siguiente manera o a través de`Autopsy`.
...
Para una evaluación integral, enviemos este hash a Virustotal

![](https://academy.hackthebox.com/storage/modules/237/image94.png)

Navegando al`Scheduled Tasks`pestaña del`Autoruns`Herramienta, descubrimos otro mecanismo de persistencia.

![](https://academy.hackthebox.com/storage/modules/237/image69.png)

### **Análisis de datos MFT con autopsia**

Mientras usamos la herramienta Autoruns para buscar persistencia, nos encontramos con la ruta de la imagen`C:\ProgramData\svchost.exe`.

Vamos a sumergirnos en`C:\ProgramData`Uso de la autopsia para encontrar este archivo.

![](https://academy.hackthebox.com/storage/modules/237/image54.png)

¿Puedes detectar alguna irregularidad? Timestomping es una técnica astuta donde los adversarios modifican las marcas de tiempo de un archivo para combinarse con los archivos circundantes, lo que hace que la detección sea un desafío para las herramientas e investigadores forenses. Accediendo a los metadatos del archivo (`File Metadata`pestaña), podemos identificar el`MFT (Master File Table) attributes`, que revelará la fecha de modificación genuina.

En particular, hay una discrepancia al contrastar el`$FILE_NAME` `MFT Modified`valor con el`$STANDARD_INFORMATION` `File Modified`valor.

El`$STANDARD_INFORMATION` `File Modified`La marca de tiempo es lo que un usuario generalmente encuentra al ver las propiedades del archivo. Esto podría llevar a alguien a creer que el archivo ha estado presente por un tiempo y podría no estar relacionado con ninguna actividad reciente. Sin embargo,`$FILE_NAME` `MFT Modified`contiene la auténtica marca de tiempo, revelando el historial real del archivo.

![](https://academy.hackthebox.com/storage/modules/237/image5__.png)

### **Análisis de datos de SRUM con autopsia**

Reflexionando sobre nuestros hallazgos, recordamos que el ejecutable malicioso tenía un mango abierto dirigido al`Desktop`carpeta. A través de`Autopsy`Notamos un archivo llamado`users.db`. Dadas las circunstancias, es plausible que el atacante haya tenido la intención de desviar estos datos del sistema.

![](https://academy.hackthebox.com/storage/modules/237/image61.png)

![](https://academy.hackthebox.com/storage/modules/237/image42.png)

Para validar nuestra hipótesis, examinemos`Data Artifacts`y acceder al`Run Programs`sección. Nuestro enfoque principal para el análisis de metadatos de red se basa en`SRUDB.dat`.

![](https://academy.hackthebox.com/storage/modules/237/image40.png)

![](https://academy.hackthebox.com/storage/modules/237/image18.png)

`430526981`Los bytes pueden haber sido exfiltrados.

### **Análisis de datos de triaje rápido: registros de eventos de Windows (motosierra)**

En nuestra búsqueda de comprender los eventos que ocurrieron, empleemos el[Motosiería](https://github.com/WithSecureLabs/chainsaw)utilidad (que reside en`C:\Users\johndoe\Desktop\chainsaw`) para analizar los registros de eventos de Windows disponibles en`C:\Users\johndoe\Desktop\kapefiles\auto\C%3A\Windows\System32\winevt\Logs`como sigue. Nuestro objetivo es identificar eventos clave que ocurrieron durante nuestra línea de tiempo de incidentes.

Al juntar la evidencia, podemos construir una narración integral del ataque, desde su inicio hasta su culminación.
...
Los resultados estarán disponibles dentro del`C:\Users\johndoe\Desktop\chainsaw\output_csv`directorio del objetivo de esta sección.

Al examinar`sigma.csv`(elegir`Fixed width`en`Separator Options`), observamos las siguientes alertas, entre otras relacionadas con el incidente.

- **Carga de huelga de cobalto por RunDLL32**
    
    ![](https://academy.hackthebox.com/storage/modules/237/image86.png)
    
- **Huelga de cobalto llamado Pipe**
    
    ![](https://academy.hackthebox.com/storage/modules/237/image77.png)
    
    ![](https://academy.hackthebox.com/storage/modules/237/image98.png)
    
    ![](https://academy.hackthebox.com/storage/modules/237/image41.png)
    
    La funcionalidad de tubería nombrada de Cobalt Strike permite la comunicación encubierta entre adversarios y sistemas comprometidos, facilitando las actividades posteriores a la explotación de manera sigilosa.
    
- **UAC (control de la cuenta de usuario) Escalación de derivación/privilegio abusando de Fodhelper.exe**
    
    ![](https://academy.hackthebox.com/storage/modules/237/image29.png)
...
- **Acceso LSASS**
    ...
    ![](https://academy.hackthebox.com/storage/modules/237/image70.png)
    
- **Ejecución de Windows PowerShell**
    
    ![](https://academy.hackthebox.com/storage/modules/237/image17.png)
...
, observamos que se creó un nuevo usuario (
...
También podemos identificar la creación de nuevos usuarios a través de
...
### **Análisis de datos rápidos de triaje: archivos de FECT (PECMD)**

Ahora vamos a sumergirnos en el historial de ejecución del sistema analizando los archivos de Fetch (disponibles en`C:\Users\johndoe\Desktop\kapefiles\auto\C%3A\Windows\Prefetch`) con`PECmd.exe`.
...
### **Análisis de datos de triaje rápido - USN Journal (USN.PY)**

Dentro del`USN journal`(disponible en`C:\Users\johndoe\Desktop\kapefiles\ntfs\%5C%5C.%5CC%3A\$Extend\$UsnJrnl%3A$J`), podemos identificar todos los archivos que fueron creados o eliminados durante el incidente.
...
Actividades sospechosas tuvieron lugar aproximadamente entre`2023-08-10 09:00:00`y`2023-08-10 10:00:00`.

Para ver el CSV usando PowerShell en alineación con nuestra línea de tiempo, podemos ejecutar:

Escenario forense digital práctico

```powershell
PS C:\Users\johndoe> $time1 = [DateTime]::ParseExact("2023-08-10 09:00:00.000000", "yyyy-MM-dd HH:mm:ss.ffffff", $null)PS C:\Users\johndoe> $time2 = [DateTime]::ParseExact("2023-08-10 10:00:00.000000", "yyyy-MM-dd HH:mm:ss.ffffff", $null)PS C:\Users\johndoe> Import-Csv -Path C:\Users\johndoe\Desktop\usn_output.csv | Where-Object { $_.'FileName' -match '\.exe$|\.txt$|\.msi$|\.bat$|\.ps1$|\.iso$|\.lnk$' } | Where-Object { $_.timestamp -as [DateTime] -ge $time1 -and $_.timestamp -as [DateTime] -lt $time2 }timestamp                  filename                              fileattr                    reason
---------                  --------                              --------                    ------
...
Si miramos lo suficientemente cuidadosamente, notaremos que`flag.txt`fue eliminado.

### **Análisis de datos de triaje rápido - MFT/PageFile.Sys (MftecMD/Autopsy)**

Podemos aprovechar`MFT`en un intento de recuperarse`flag.txt`. Desafortunadamente, la tabla MFT de la máquina afectada no está disponible.

Por integridad, trabajemos en la tabla MFT de otro sistema (disponible en`C:\Users\johndoe\Desktop\files\mft_data`) dónde`flag.txt`también fue eliminado.

Nuestro paso inicial implica correr`MFTEcmd`para analizar el`$MFT`archivo, seguido de la búsqueda de`flag.txt`dentro del informe.
...
La salida proporciona la ubicación de`flag.txt`en el sistema (`\Users\johndoe\Desktop\reports`).

Vamos a acceder ahora al`MFT`archivo (`C:\Users\johndoe\Desktop\files\mft_data`) usando`MFT Explorer`(disponible en`C:\Users\johndoe\Desktop\Get-ZimmermanTools\net6\MFTExplorer`)

En el`Desktop`, dentro del`reports`carpeta, descubrimos`flag.txt`marcado con el`Is deleted`atributo.

![](https://academy.hackthebox.com/storage/modules/237/image31.png)

Cuando los archivos se eliminan de un volumen del sistema de archivos NTFS, su`MFT`Las entradas se marcan como gratuitas y pueden reutilizarse, pero los datos pueden permanecer en el disco hasta que se sobrescriben. Es por eso que la recuperación no siempre es posible.

En el caso del sistema comprometido, el archivo se sobrescribió (por eso utilizamos el`MFT`Tabla de otro sistema para el ejercicio de recuperación), pero las partes de su contenido se conservaron en`pagefile.sys`.

`pagefile.sys`es un archivo de sistema designado en Windows que complementa la RAM de su computadora. Cuando la RAM se acerca a su capacidad, el sistema descarga datos menos críticos, como ciertos archivos y aplicaciones, al archivo de página.

Con el conocimiento del contenido parcial del archivo, podemos recorrer el disco y recuperar nuestra bandera de`pagefile.sys`a través de`Autopsy`.
...
# **Construyendo una línea de tiempo de ejecución**

Dado que el incidente ocurrió entre`09:13`y`09:30`, podemos usar`Autopsy`para mapear las acciones del atacante cronológicamente.

Detrás de escena, la autopsia emplea[Plaso](https://github.com/log2timeline/plaso).

Hagamos las siguientes selecciones:

- Límite de los tipos de eventos a:
    - `Web Activity`: Todo
    - `Other`: Todo
- Establecer tiempos de visualización en: GMT / UTC
    - `Start`: 10 de agosto de 2023 9:13:00 am
    - `End`: 10 de agosto de 2023 9:30:00 am

![](https://academy.hackthebox.com/storage/modules/237/timeline.png)

Esto nos permitirá generar una línea de tiempo que detalla las acciones realizadas por el actor malicioso.

![](https://academy.hackthebox.com/storage/modules/237/image66.png)

Vista de línea de tiempo de la lista:

![](https://academy.hackthebox.com/storage/modules/237/image89.png)

Al aplicar filtros específicos, podemos identificar los archivos accedidos o establecidos durante esta ventana en particular.

---

# **La línea de tiempo de ataque real**

Aquí están el`real`Acciones tomadas por el atacante (es decir,`not identified through digital forensics`). Según lo que ha aprendido hasta este punto, intente reconocer y identificar cualquiera de estas acciones que permanezcan sin ser detectadas en esta sección.
